...
class Conv1d_batchnorm(torch.nn.Module):
    ...
    def __init__(self, num_in_filters, num_out_filters, kernel_size, stride=1, activation='relu'):
        ...
        self.conv1 = torch.nn.Conv1d(num_in_filters, num_out_filters, kernel_size=kernel_size, stride=stride, padding=padding_val)
...
class Multiresblock(torch.nn.Module):
    """
    MultiRes Block – fixed according to MultiResUNet paper (W = alpha * U)
    """
    def __init__(self, num_in_channels, num_filters, alpha=1.67, kernel_size=[3, 5, 7]):
        super().__init__()

        # Paper formula: W = α × U
        W = int(alpha * num_filters)

        # Split channels into 3 branches
        f1 = int(W * 0.167)
        f2 = int(W * 0.333)
        f3 = W - f1 - f2   # đảm bảo tổng đúng W

        self.conv_1 = Conv1d_batchnorm(num_in_channels, f1, kernel_size=kernel_size[0], activation='relu')
        self.conv_2 = Conv1d_batchnorm(f1, f2, kernel_size=kernel_size[1], activation='relu')
        self.conv_3 = Conv1d_batchnorm(f2, f3, kernel_size=kernel_size[2], activation='relu')

        self.total_filters = f1 + f2 + f3
    ...
    def forward(self, x):
        ...
        concat = torch.cat([res1, res2, res3], axis=1)  # giữ nguyên axis theo yêu cầu
...
class MultiResUnet(torch.nn.Module):
    """
    MultiResUNet – Fixed according to MultiResUNet paper
    """
    def __init__(self, para, alpha=1.67):
        ...
        # Encoder
        for i in range(L):
            self.encoder_layer.append(Multiresblock(en_str[i][0], self.para["Filter nums"][i], alpha=self.alpha, kernel_size=self.para["Block kernel"]))
        ...
        # Bottleneck
        self.bottle_neck = Multiresblock(bn_str[0], self.para["Filter nums"][-1], alpha=self.alpha, kernel_size=self.para["Block kernel"])
        ...
        for i in range(L):
            ...
            self.decoder_layer.append(Multiresblock(de_str[i][0], self.para["Filter nums"][filter_num_idx], alpha=self.alpha, kernel_size=self.para["Block kernel"]))

        # Final layer
        last_filter_arg = self.para["Filter nums"][0]
        actual_last_out_ch = self.get_block_actual_output(last_filter_arg)  # chuẩn paper
    ...
    def get_block_actual_output(self, num_filters):
        """Số channel thực tế theo paper: W = α × U, rồi chia 16.7% – 33.3% – 50%"""
        W = int(self.alpha * num_filters)
        f1 = int(W * 0.167)
        f2 = int(W * 0.333)
        f3 = W - f1 - f2
        return f1 + f2 + f3
    ...
    def structure_calculate(self):
        ...
        if isinstance(current_input_ch, list):
            current_input_ch = 1
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(len(self.decoder_layer)):
            ...
            concat = torch.cat([upsampled, skip_connection], axis=1)  # giữ nguyên axis theo yêu cầu
