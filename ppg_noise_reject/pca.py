...
def pca_analysis(latent,disease, n_components=3):
    ...
    idx0 = np.where(disease == 0)[0]
    idx1 = np.where(disease == 1)[0]
    idx2 = np.where(disease > 1)[0]
    ax1.scatter(latent_pca[idx0, 0], latent_pca[idx0, 1], label = "Healthy")
    ax1.scatter(latent_pca[idx1, 0], latent_pca[idx1, 1], label = "AF")
    ...
    ax2.scatter(latent_pca[idx1, 0], latent_pca[idx1, 1], latent_pca[idx1, 2], label = "AF")
...
pca_analysis(latent_train,disease_train)