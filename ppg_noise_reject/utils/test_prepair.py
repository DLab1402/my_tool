...
class MainWindow(QWidget):
    ...
    def load_json(self, item):
        ...
        try:
            ...
            min_val = min(self.cur_data["PPG"])
            max_val = max(self.cur_data["PPG"])

            self.cur_sig = [(x - min_val)/(max_val - min_val) for x in self.cur_data["PPG"]]
            self.cur_label = [1 if x > 1 else x for x in self.cur_data["Label"]]
            self.cur_peak = [[],[]]
            self.cur_valid = []
            if "Test" in self.cur_data:
                self.cur_temp = self.cur_data["Test"]
                for item in self.cur_data["Test"]:
    ...
    def back_action(self):
        if self.cur_index > 0:
            ...
            self.cur_rect.set_x(p1)
            self.cur_rect.set_width(p2-p1)
            self.plot2_handle(self.cur_data["Test"][self.cur_index]["Temp"])
            self.plot1.draw()

    def next_action(self):
        if self.cur_index < len(self.cur_data["Test"]) - 1:
            self.cur_index += 1
            p1 = self.cur_data["Test"][self.cur_index]["Pos"][0]
            p2 = self.cur_data["Test"][self.cur_index]["Pos"][1]
            self.cur_rect.set_x(p1)
...
if __name__ == "__main__":
    ...
    window = MainWindow("H:\\My Drive\\data_set_ppg_reject4\\test")
    window.show()
    sys.exit(app.exec())