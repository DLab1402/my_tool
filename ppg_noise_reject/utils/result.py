...
def loss_disstribution(path):
    file = os.path.join(path,"loss.json")
...
def PCA_space(path):
    file = os.path.join(path,"test_latent.json")
    with open(file, "r") as f:
        data1= json.load(f)
        latent = np.array(data1["latent"])
        print(latent.shape)
        idx0 = np.where(np.array(data1["disease"]) == 0)[0]
        idx1 = np.where(np.array(data1["disease"]) == 1)[0]
        idx2 = np.where(np.array(data1["disease"]) > 1)[0]
    ...
    file = os.path.join(path,"val_latent.json")
    with open(file, "r") as f:
        data1= json.load(f)
        latent = np.array(data1["latent"])
        print(latent.shape)
